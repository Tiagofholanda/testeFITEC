...
# Função para carregar credenciais de login de um arquivo JSON
...
# Inicializando a sessão de estado para manter o login
if 'logged_in' not in st.session_state:
    st.session_state['logged_in'] = False

# Título da aplicação
st.title("WebGIS Interativo de gestão da informação")

# Caminho relativo para o arquivo de credenciais
credentials_file = os.path.join(base_dir, "config", "credentials.json")
credentials = load_credentials(credentials_file)

# Verificar login apenas se o usuário ainda não estiver logado
if not st.session_state['logged_in']:
    ...
    if st.sidebar.button("Entrar"):
        if check_login(username, password, credentials):
            st.session_state['logged_in'] = True  # Definir login como verdadeiro
            st.sidebar.success("Login realizado com sucesso!")
        else:
            st.sidebar.error("Usuário ou senha incorretos.")
else:
    # Aqui segue o código principal da aplicação, após o login bem-sucedido
    st.title("Bem-vindo ao WebGIS Interativo!")

    # Caminho para o logo da empresa (relativo)
    ...
    # Caminho para o arquivo Excel (relativo)
    excel_file = os.path.join(base_dir, "Dados", "PONNOT_01_1.xlsx")
    ...
    if df is not None:
        ...
        df = convert_utm_to_latlon(df, utm_zone="23S")
